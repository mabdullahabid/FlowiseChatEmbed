...
import { ClipboardIcon, ShareIcon, ThumbsDownIcon, ThumbsUpIcon } from '../icons';
...
export const ShareTextButton = (props: RatingButtonProps) => {
  return (
    <button
      disabled={props.isDisabled || props.isLoading}
      {...props}
    ...
    >
      <Show when={!props.isLoading} fallback={<Spinner class="text-white" />}>
        <ShareIcon color={props.feedbackColor ?? defaultFeedbackColor} class={'send-icon flex ' + (props.disableIcon ? 'hidden' : '')} />
      </Show>
    </button>
  );
};
